...
pEulerAAL <- read.csv("../data/Statistics/ccsize_euler_aal.csv" )
pFactAAL  <- read.csv("../data/Statistics/ccsize_fact_aal.csv" )
pRkAAL <- read.csv("../data/Statistics/ccsize_rk4_aal.csv" )
pTendAAL <- read.csv("../data/Statistics/ccsize_tend_aal.csv" )
  
pEulerDKT <- read.csv("../data/Statistics/ccsize_euler_dkt31.csv" )
pFactDKT  <- read.csv("../data/Statistics/ccsize_fact_dkt31.csv" )
pRkDKT <- read.csv("../data/Statistics/ccsize_rk4_dkt31.csv" )
pTendDKT <- read.csv("../data/Statistics/ccsize_tend_dkt31.csv" )
...
data <- data.frame(LargestConnectedComponent=values, Density=dens, Algorithm=as.factor(alg), ID=as.factor(id), Labels=as.factor(label) )

pathplot <- ggplot( data, aes(x=Density, y=LargestConnectedComponent, colour=Algorithm, group=ID ) ) + geom_line() + ggtitle("Largest conncted component in constant density subgraphs") + theme(plot.title = element_text(lineheight=0.5)) + facet_grid(. ~ Labels)

png( filename="../data/Statistics/size_plot.png", width=1000, height=300 )
print(pathplot)
dev.off()
...
for ( i in c(1:ln) ) {
  ...
  if ( (i <= dim(v3)[2]) && (var(v3[,i]) > minvar) ) {
    dat <- data.frame(path=v3[,i],id=ids)
    picc1 <- ICCest(id,path,dat)
    icc3[i] <- picc1$ICC
  }
  
  if ( (i <= dim(v4)[2]) && (var(v4[,i]) > minvar) ) {
    dat <- data.frame(path=v4[,i],id=ids)
    picc1 <- ICCest(id,path,dat)
    icc4[i] <- picc1$ICC
  }
  
  if ( i <= dim(v5)[2] && (var(v5[,i]) > minvar) ) {
    dat <- data.frame(path=v5[,i],id=ids)
    picc1 <- ICCest(id,path,dat)
    icc5[i] <- picc1$ICC
  }
  
  if ( i <= dim(v6)[2] && (var(v6[,i]) > minvar) ) {
    dat <- data.frame(path=v6[,i],id=ids)
    picc1 <- ICCest(id,path,dat)
    icc6[i] <- picc1$ICC
  }
  
  if ( i <= dim(v7)[2] && (var(v7[,i]) > minvar) ) {
    dat <- data.frame(path=v7[,i],id=ids)
    picc1 <- ICCest(id,path,dat)
    icc7[i] <- picc1$ICC
  }
  
  if ( i <= dim(v8)[2] && (var(v8[,i]) > minvar) ) {
