...
m1 <- colMeans(intraEulerAAL[,2:dim(intraEulerAAL)[2]])
m2 <- colMeans(intraFactAAL[,2:dim(intraFactAAL)[2]])
m3 <- colMeans(intraRkAAL[,2:dim(intraRkAAL)[2]])
m4 <- colMeans(intraTendAAL[,2:dim(intraTendAAL)[2]])

m5 <- colMeans(interEulerAAL[,2:dim(interEulerAAL)[2]])
m6 <- colMeans(interFactAAL[,2:dim(interFactAAL)[2]])
m7 <- colMeans(interRkAAL[,2:dim(interRkAAL)[2]])
m8 <- colMeans(interTendAAL[,2:dim(interTendAAL)[2]])
...
m9 <- colMeans(intraEulerDKT[,2:dim(intraEulerDKT)[2]])
m10 <- colMeans(intraFactDKT[,2:dim(intraFactDKT)[2]])
m11 <- colMeans(intraRkDKT[,2:dim(intraRkDKT)[2]])
m12 <- colMeans(intraTendDKT[,2:dim(intraTendDKT)[2]])

m13 <- colMeans(interEulerDKT[,2:dim(interEulerDKT)[2]])
m14 <- colMeans(interFactDKT[,2:dim(interFactDKT)[2]])
m15 <- colMeans(interRkDKT[,2:dim(interRkDKT)[2]])
m16 <- colMeans(interTendDKT[,2:dim(interTendDKT)[2]])
...
png( filename="../data/Statistics/dice_overlap_plot.png", width=500, height=300 )
print(diceplot)
dev.off()
