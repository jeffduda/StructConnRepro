...
pEulerAAL <- read.csv("../data/Statistics/pathlength_euler_aal.csv" )
pFactAAL  <- read.csv("../data/Statistics/pathlength_fact_aal.csv" )
pRkAAL <- read.csv("../data/Statistics/pathlength_rk4_aal.csv" )
pTendAAL <- read.csv("../data/Statistics/pathlength_tend_aal.csv" )
  
pEulerDKT <- read.csv("../data/Statistics/pathlength_euler_dkt31.csv" )
pFactDKT  <- read.csv("../data/Statistics/pathlength_fact_dkt31.csv" )
pRkDKT <- read.csv("../data/Statistics/pathlength_rk4_dkt31.csv" )
pTendDKT <- read.csv("../data/Statistics/pathlength_tend_dkt31.csv" )
...
data <- data.frame(CharacteristicPathLength=values, Density=dens, Algorithm=as.factor(alg), ID=as.factor(id), Labels=as.factor(label) )

pathplot <- ggplot( data, aes(x=Density, y=CharacteristicPathLength, colour=Algorithm, group=ID ) ) + geom_line() + ggtitle("Characteristic path length in constant density subgraphs") + theme(plot.title = element_text(lineheight=0.5)) + facet_grid(. ~ Labels) 

png( filename="../data/Statistics/path_plot.png", width=1000, height=300 )
print(pathplot)
dev.off()
