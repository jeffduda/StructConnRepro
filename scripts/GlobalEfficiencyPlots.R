...
pEulerAAL <- read.csv("../data/Statistics/gefficiency_euler_aal.csv" )
pFactAAL  <- read.csv("../data/Statistics/gefficiency_fact_aal.csv" )
pRkAAL <- read.csv("../data/Statistics/gefficiency_rk4_aal.csv" )
pTendAAL <- read.csv("../data/Statistics/gefficiency_tend_aal.csv" )
  
pEulerDKT <- read.csv("../data/Statistics/gefficiency_euler_dkt31.csv" )
pFactDKT  <- read.csv("../data/Statistics/gefficiency_fact_dkt31.csv" )
pRkDKT <- read.csv("../data/Statistics/gefficiency_rk4_dkt31.csv" )
pTendDKT <- read.csv("../data/Statistics/gefficiency_tend_dkt31.csv" )
...
data <- data.frame(GlobalEfficiency=values, Density=dens, Algorithm=as.factor(alg), ID=as.factor(id), Labels=as.factor(label) )

pathplot <- ggplot( data, aes(x=Density, y=GlobalEfficiency, colour=Algorithm, group=ID ) ) + geom_line() + ggtitle("Global efficiency in constant density subgraphs") + theme(plot.title = element_text(lineheight=0.5)) + facet_grid(. ~ Labels)


png( filename="../data/Statistics/geff_plot.png", width=1000, height=300 )
print(pathplot)
dev.off()
