...
\processtable{Functional data analysis is used along with permutation
  testing to look for differences in dice overlap measures between graphs 
...
set (* indicates significance at $p = 0.05$).\label{tab:dicep}}
{
...
\processtable{Functional data analysis is used along with permutation
  ...
  subject is used. For each metric, the upper-triangular values are p-values for
  the AAL labels while the lower-triangular values were generated with
  the DKT31 label set (* indicates significance at $p = 0.05$).\label{tab:permtesting}}
{\begin{tabular}{l | llll | llll }
\midrule
...
generated from different fiber tracking algorithms. Upper triangular p-values
are for the AAL labels, while lower triangular are for the DTK31 label
set  (* indicates significance at $p = 0.05$).\label{tab:richp}}
{
\begin{tabular}{l | llll }
\midrule
